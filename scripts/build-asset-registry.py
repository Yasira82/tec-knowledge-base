...
def classify_tier(cid_num):
    ...
    if n == 132:  # Service Extraction & Modular Architecture Policy (ADR-011 detail) — current-state governance
        return 'tier-1-constitutional-runtime'
    if n == 133:  # Platform Adoption & Growth Governance — current-state governance/strategy (like C-77/C-78)
        return 'tier-1-constitutional-runtime'
...
def infer_authority_scope(tier, cid_num):
    ...
    if 100 <= cid_num <= 115:
        return 'app'
    if 124 <= cid_num <= 131:  # NBF/Epic/Legend/Elite/VIP/Insure/Titan/Brookfield app charters
        return 'app'
...
def infer_type(cid_num, tier, role):
    ...
    if 100 <= cid_num <= 115:
        return 'charter'
    if 124 <= cid_num <= 131:  # app charters titled "... RUNTIME"
        return 'charter'
