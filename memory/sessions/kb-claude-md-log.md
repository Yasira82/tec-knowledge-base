# KB change log — the "Session N Additions" blocks, Sessions 12 → 46

> Moved out of the KB's `CLAUDE.md` on 2026-09-24 (KB remediation step 6, audit F16). The
> text is unchanged. It was an append-only log inside the file every session loads, and its
> counts (gates, assets, versions) were each true only on the day they were written.
> This is a **historical record**; for current state read C-02, for how to work read `CLAUDE.md`.
...
| Sensor sync | `stream-health.service.ts` + tec-core-backend `scripts/verify-runtime.mjs` | Added `commerce-subscription` (and `identity-nexus`, from the Nexus resume fix) to the `payment.completed.v1` EXPECTED map. |
