...
## P1 — DOCUMENTED → Pending Code Commit (Code Verified — June 2026)

> **Truth State:** `[Current State]` | **Verification:** `[Code Verified]`
> **Implementation Guide:** C-81___P1_FIXES_IMPLEMENTATION.md — complete code ready

| ID | المشكلة | المصدر | الحل | الحالة |
|----|---------|--------|------|--------|
| **NEW-K** | **Duplicate Health Polling** — BackendOfflineBanner + BackendStatus كلاهما يعمل polling مستقل كل 30s | `Tec-App/src/components/` | `PlatformHealthContext.tsx` — كود كامل في C-81 | **📋 DOCUMENTED** |
| **NEW-L** | **Gateway Timeout Mismatch** — Gateway 30s vs Frontend 5s → 499 ghost failures | `tec-api-gateway/src/modules/proxy/proxy.service.ts` | `timeout: 10000, proxyTimeout: 10000` — في C-81 | **📋 DOCUMENTED** |
| **NEW-N** | **Redis Silent Failure** — `client.on('error', () => {})` يكتم كل الأخطاء | `tec-api-gateway/src/modules/redis/` | 5 event listeners — كود كامل في C-81 | **📋 DOCUMENTED** |

---

## P1-C — DOCUMENTED → Pending Code Commit

| ID | المشكلة | المصدر | الحل | الحالة |
|----|---------|--------|------|--------|
| **NEW-O** | **Health Endpoint Missing Detail** — `/api/health` يُعيد `{ "status": "ok" }` فقط — zero evidence at incident time | `tec-api-gateway/src/modules/health/` | `GET /api/health/details` (x-internal-key) — NestJS controller + service كاملين في C-81 | **📋 DOCUMENTED** |
...
```
P0 Open:  0
P1 Documented: 4  📋  (NEW-K, NEW-L, NEW-N, NEW-O — complete code in C-81, pending commit to repos)
...
Next:
  1. Apply C-81 to repos: NEW-L → NEW-N → NEW-O → NEW-K (order in C-81)
  2. Run tests in Tec-core-backend and Tec-App
  3. Merge PRs → mark violations VERIFIED in C-40
  4. External Re-Audit → target 9.0–9.5/10
  5. Fix NEW-M (service-registry.ts) — P2
