...
| 4 | **Estate: a registration started from the Hub (Mode 1) is lost** — the form lives in page state, so after the Hub payment the user returns to an empty form: paid 3π, no property. Standalone (Mode 2) works. Carry the draft across the round trip, or register from the payment's event as Assets now does | Real π taken, nothing recorded — for any user who opens Estate from the Hub | 56r |
