...
| 4 | **Assets — deploy #336** (paid `provision` verified with payment-service; owner routes require the session user). **First** set `PAYMENT_SERVICE_URL` on asset-service (copy from commerce), or paid registrations answer 503. #334 + Tec-Assets #59 are live (receipts, consumer, approve compares Pi's amount) | Until #336 deploys, a domain / property can be registered without paying, and a listing repriced or delisted by anyone | 56r |
