...
|---|---|---|---|
| 1 | **Phone checks**: Hub keeps the name a day after sign-in (#256)? "Try again" recovers a silent Pi (#253)? **The middleware never ran** in 20 apps + the template (root `middleware.ts`, `src/app`): no page guard, **no CSRF enforcement** in production (C-123 §11). Connection #83 · DX #39 · Alert #40 move it into `src/` with **no** page guard (a Hub-SSO redirect stranded Quest visits and was rolled back) — merged and promoted. **Quest/campaign sign-in:** tec-app #258 signs each app link on the Hub before the visit leaves (C-123 §12) — merge, then open DX · Alert · Connection from the Quest and the campaign: the name should show and Pi should still count the visit | Only a device can answer | 56p · 56q · 56r |
